...
{
    return textureArrayBicubic(lightmapSampler, vec3(coords.x, coords.y, page)).rgb;
}
...
void main()
{
    // Clipping first!
        ...
        {
            if (!ClipPlaneTest(l_eyePosition, p3d_WorldClipPlane[i]))
            {
    ...
    */
    ...
    vec3 diffuseLighting = vec3(1);
        ...
        //vec3 proj = vec3(0);
        //float depthCmp = 0.0;
        //int cascade = FindCascade(l_pssmCoords, proj, depthCmp);

        //outputColor = vec4(0, 0, 0, 1);
        //outputColor[cascade] = 1;
        //return;
    #endif
