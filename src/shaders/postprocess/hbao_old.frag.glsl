...
uniform vec3 NoiseScale_MaxRadiusPixels;// = vec2(1024.0, 768.0) / 4.0;
...
void ComputeSteps(inout vec2 stepSizeUv, inout float numSteps, float rayRadiusPix, float rand)
{
    ...
    float maxNumSteps = NoiseScale_MaxRadiusPixels.z / stepSizePix;
    if (maxNumSteps < numSteps)
    {
        // Use dithering to avoid AO discontinuities
        numSteps = floor(maxNumSteps + rand);
        numSteps = max(numSteps, 1);
        stepSizePix = NoiseScale_MaxRadiusPixels.z / numSteps;
    }
...
void main(void)
{
	...
	vec3 random = texture(noiseSampler, l_texcoord.xy * NoiseScale_MaxRadiusPixels.x).rgb;
